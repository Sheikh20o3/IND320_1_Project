...
def _load_energy_csv_generic(
    ...
    # Aggregate to one row per hour (sum over groups)
    out = (
...
def load_consumption_series(price_area: str, year: int) -> pd.DataFrame:
    ...
    if not CONSUMPTION_CSV.exists():
        st.error(
            ...
            f"Expected path: '{CONSUMPTION_CSV}'."
        )
        return pd.DataFrame(columns=["time", "energy_kwh"])
    ...
    if df.empty:
        st.warning(
            f"No consumption data for {price_area} in {year} "
            f"in CSV file '{CONSUMPTION_CSV.name}'."
        )
...
def load_production_series(price_area: str, year: int) -> pd.DataFrame:
    ...
    if not paths:
        st.error(
            "Production CSV files not found.\n\n"
    ...
    if df.empty:
        st.warning(
            f"No production data for {price_area} in {year} "
            f"in the production CSV files."
        )
...
def fetch_energy_series(
    ...
    Returns DataFrame with ['time', 'energy_kwh'].
    """
...
def prepare_endog_and_exog(
    ...
    We limit the training dataset to the last `max_train_points` observations
    to keep SARIMAX fast enough for Streamlit Cloud.
    """
    # Energy to time index
    ...
    if y_train.empty:
        raise ValueError("Training period is empty – adjust the dates.")

    # Forecast horizon (can extend beyond the data range)
    ...
    if exog_keys and df_met is not None:
        ...
        # Reindex to cover both training and forecast period
